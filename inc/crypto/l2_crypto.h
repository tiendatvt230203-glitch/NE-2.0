...
/* Outer EtherType + worker ID + nonce + GCM tag. The original EtherType
 * remains inside the encrypted payload, so it adds two bytes as well. */
#define L2_CRYPTO_DATA_OVERHEAD (2u + 1u + PACKET_CRYPTO_NONCE_BYTES + AES_GCM_TAG_SIZE)
...
/* Shared authenticated fragment format for UDP and oversized ICMP. */

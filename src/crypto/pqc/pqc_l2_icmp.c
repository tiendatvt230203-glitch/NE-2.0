...
static int icmp_encrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt,
                        uint32_t *pkt_len)
{
    int n;
...
static int icmp_decrypt(struct packet_crypto_ctx *ctx, uint8_t *pkt,
                        uint32_t *pkt_len)
{
    int n;
...
CRYPTO_OPS_PLAIN(crypto_opt_l2_pqc_icmp_ops, icmp_encrypt, icmp_decrypt)
